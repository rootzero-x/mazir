...
export default function MessageBubble({ message, isMine, previousMessageSameSender, onRetry }: MessageBubbleProps) {
    ...
    // Fallback to 'createdAt' (legacy) if created_at is missing, casting to any to avoid TS error
    const time = formatTime(message.created_at || (message as any).createdAt || "");

    // 2. Strict Username Logic (No "Unknown" fallback)
    // We trust the backend to provide 'username'
    const username = message.username || message.sender?.username || "";
    ...
    return (
        ...
        >
            ...
            {!isMine && !previousMessageSameSender && (
                <div className="h-8 w-8 shrink-0 rounded-full bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-400 select-none border border-slate-700">
                    {avatarLetter}
                </div>
            )}
